...
**Detect-cfg build tooling** lives in `tools/ddr-autodetect/` (**gitignored, dev-only**): `build.sh` cross-assembles the arm64/arm32 `probe.S.in`/`reboot.S.in` payloads (clang) and packs each SoC's `DDR自动探测.cfg`. A full rebuild needs the vendor rkbin DDR bin (not in this repo, per-machine path); `OTP_ONLY=1 bash build.sh` instead rebuilds ONLY the otpdump payload and swaps it into the shipped cfg (`lib/repack_otpprobe.py`), carrying every other record over still-RC4-encrypted so it cannot drift. Only the generated cfg ships in `DDRTestFiles/<soc>/`; regenerate via `bash tools/ddr-autodetect/build.sh` when payloads/addresses change. arm32 payloads must be fully inline (no `bl`/forward branches — the raw `.text` extractor doesn't resolve R_ARM_CALL relocations).
