...
- **GUITests** — the GUI's step pipeline: which card an engine log entry lands on, and what happens when a completion entry goes missing. Reachable the same way `CLIContractTests` is.
...
`DdrDetector` (DDRUSB) identifies a board's DRAM geometry **over pure USB** (no serial) and detect+test now run as ONE atomic operation (`MainViewModel.performDetectThenTest`), never detect alone. Plugging a device in never probes or drives it: `pollDevices` is bookkeeping only. Every run starts at the 「开始」 button, and `startTest()` has exactly ONE caller — it orchestrates a detect-then-test when the device is unprobed and supported, else runs the already-selected cfg. (An 自动测试 switch used to fire that same path on plug-in. It went away when the operator gained a device picker: "which board did this arrival mean" has no good answer once more than one board can be on the bus, and a tool that drives hardware on its own is a risk surface for a capability whose default was off.) A cfg is set only by a unique detect hit (`uniqueByCoarse`/`uniqueByTieBreak`) or manual selection — there is no default-first-file fallback; `.ambiguous`/`.none` stop and leave `selectedFileID` unset for the operator to pick. This replaced the old always-probe-on-plug-in model (`runDetectThenMaybeTest`/`maybeAutoTest`/`maybeLaunchAutoDetect`). Supported SoCs: RK3568/3566 (0x350A), RK3588 (0x350B), RK3576 (0x350E), RK3288 (0x320A) — see `DetectProfiles`. Detect flow:
...
4. The detect cfg packages Boot + ddrbin + osregdump + otpdump + reboot; `DdrDetector` drives them itself (NOT via `TestExecutionEngine`). Both probe items go through `DdrDetector.runProbeItem`, which waits for the DEVICE to report the item finished (`testDeviceReady` word1). **Completion is a device STATE, not a content marker** — the old code returned as soon as the captured text parsed, which on both an RK3566 and an RK3576 left the loader ~560ms short of returning, and the next `downloadItem` then went to a firmware not yet back in its command loop. The polling rule is transcribed from `EyescanRunner` (measured there): while printf hands back data the item is still producing and an opcode-0 contends with it on the same device-side loop, so status is probed ONLY once the ring goes quiet. A status error or the deadline is never fatal — the capture is returned with `returned == false`, the behaviour this code had before it looked at status at all. Measured effect: osregdump 3.83s → 1.46s, whole detect 7.4s → 5.6s (RK3566) and 8.5s → 6.7s (RK3576), with byte-identical geometry/candidates/OTP. `CfgRepository.discoverTestFiles()` EXCLUDES any `…自动探测.cfg` so it's never user-selectable / run as a test.
...
**GUI toggle** (`MainViewModel`, in-memory, not persisted): `autoDetectEnabled` (default ON — gates whether `startTest()` attempts a detect at all for an unprobed, supported device; off ⇒ `startTest()` just runs the manually selected cfg).

**Device targeting.** The toolbar shows a picker when more than one board is on the bus, labelling each by the SOCKET it sits in (`RK3566 · 1.4`) — Maskrom devices report no serial and the USB address is reassigned on every re-enumeration, so `deviceID`'s port-chain field is the only stable discriminator. `selectedDeviceID` was honoured by every path long before anything could set it. Its `didSet` drops the held handle and the whole `Connection`: the latches describe ONE board, and with two boards on the bus, unplugging the probed one hands the selection to the survivor without emptying the bus — so the "device removed" reset never ran and the next run skipped both the boot and the detect against a different board.
