...
The cfg carries two records the tool's existing engine already knows how to run:
...
Layout mirrors a real cfg so the shipping CfgBinaryParser reads it unchanged:
  ...
  0x5B6     download base (UInt32 LE) = 0xFDCC4000
  0x5C7     record[2] slot — marker zeroed so parsing stops after 2 records
  0x894     Boot payload, then osregdump payload (RC4)

Usage:
  build_detect_cfg.py <template.cfg> <boot.bin|--from-template> <probe.bin> <out.cfg>
"""
import struct, sys
...
def main():
    template, boot_arg, probe_path, out = sys.argv[1:5]
    ...
    # stop parsing after 2 records: clear the record[2] marker
    struct.pack_into("<H", buf, REC0 + 2 * SP, 0x0000)
    # download base
